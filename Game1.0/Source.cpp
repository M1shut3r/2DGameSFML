...
RenderWindow window(VideoMode(1280, 800), "2D Game SFML");
...
static sf::String U8(const std::string& text) {
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        if (Ball.WIN == 1){
            text1.setString(U8("ЭТО ОБУЧАЮЩИЙ УРОВЕНЬ, ЧТОБЫ ПЕРЕЙТИ НА СЛЕДУЮЩИЙ, ПОПАДИ МЯЧОМ В СЕРЫЙ КВАДРАТ\n ПОДСКАЗКА : КЛИКНИ НА ЭКРАН, ЧТОБЫ ДАТЬ НАПРАВЛЕНИЕ"));//задает строку тексту
            text1.setPosition(200, 0);
        }
        else {
            text1.setString(U8(std::string("Таймер:") + timeend.str()));//задает строку тексту
            text2.setString(U8(std::string("Время на прохождение уровня:") + time.str()));
